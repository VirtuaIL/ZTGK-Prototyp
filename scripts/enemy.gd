...
@export var suspicion_time: float = 1.5      # Seconds of continuous sight before CHASE
...
func take_damage(_amount: float, _source_id: int = -1, _hit_pos: Vector3 = Vector3.ZERO) -> void:
	# Enemies are invulnerable in stealth mode — this is a no-op
	# kept for API compatibility (turret lasers, etc.)
	pass
...
func _on_player_died() -> void:
	# When player resets, enemy returns to spawn
	_respawn()
...
func _create_fov_cone() -> void:
	...
	# Build a triangle-fan cone mesh
...
func _rebuild_fov_geometry(im: ImmediateMesh) -> void:
	...
	for i in range(FOV_SEGMENTS):
		...
		# Cone points in local +Z (enemy forward convention), fan from origin to arc
...
func _update_fov_cone() -> void:
	...
	# Hide cone when passive/dead/catching
	...
	# Child of enemy — just zero out local transform, inherits parent's pos/rot
